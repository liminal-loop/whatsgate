      ...
      providers: [
        // Database Adapter
        {
          ...
          useFactory: (config: ConfigService) => {
            const type = config.get('database.type', 'postgres');
...
|-------------|----------|---------|-------|--------|----------|
| **Development** | PostgreSQL | Local | Memory | Mock | Fast iteration, testing |
| **Testing** | PostgreSQL | Local | Memory | Mock | CI/CD, unit tests |
| **Staging** | PostgreSQL | Local | Redis | whatsapp-web.js | Pre-production validation |
| **Production (Small)** | PostgreSQL | Local | Memory | whatsapp-web.js | 1-3 sessions, VPS |
...
| **Database** | PostgreSQL | PostgreSQL | Single source of truth |
...
| **Time-series queries** | Native SQL/date functions |
...
- Keep `synchronize=false` outside local prototyping.
- Apply schema changes via TypeORM migrations.
- Configure pooled connections based on workload profile.
- Enable SSL in non-local deployments.
}
```
