...
export class WebhookService {
  ...
  private parseWebhookHeaders(headers: unknown): Record<string, string> {
    const source =
      typeof headers === 'string'
        ? (() => {
            try {
              return JSON.parse(headers) as unknown;
            } catch {
              return {};
            }
          })()
  ...
  async test(sessionId: string, webhookId: string): Promise<{ success: boolean; statusCode?: number; error?: string }> {
    ...
    const parsedHeaders = this.parseWebhookHeaders(webhook.headers);
